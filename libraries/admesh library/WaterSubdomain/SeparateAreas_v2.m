function AreaInfo = SeparateAreas_v2(MA,D,dx,delta_lfs,delta_A)
%% ========================================================================
...
dist = dist*dx;
dMA = sparse(I,J,dist,N,M); clear dist;
...
% dMA = double(dMA);
lfs = sparse(I,J,(abs(dMA(I,J)) + abs(D(I,J))),N,M);
%--------------------------------------------------------------------------
% Truncate based on local feature size
%--------------------------------------------------------------------------
lfs_2D = lfs;
lfs_2D(idOut) = nan;
id_L1D = lfs_2D < delta_lfs & lfs_2D > 0;
lfs_2D(id_L1D) = nan;
